import { makeStyles, Box, Container, Drawer, ListItem, List, ListItemIcon, ListItemText, Divider } from '@material-ui/core'
import { useState } from 'react'
import { Link, useLocation } from 'react-router-dom'
import { Button } from '../../styles/component'
import Hamburger from './Hamburger'
import { Home as HomeIcon, MonetizationOn, AccountBalance } from '@material-ui/icons'
                ...
                <Box display="flex" justifyContent="space-between" alignItems="center" height="60px">
                    ...
                    <nav className={classes.navSection}>
                        <ul className={classes.nav}>
                            <li>
                                <Link to="/" className={`${classes.link} ${pathname === '/' && 'active'}`}>Home</Link>
                            </li>
                            <li>
                                <Link to="/trade" className={`${classes.link} ${pathname.includes('/trade') && 'active'}`}>Trade</Link>
                            </li>
                            <li>
                                <Link to="/portfolio" className={`${classes.link} ${pathname === '/portfolio' && 'active'}`}>
                                    <Button color="primary" variant="contained" size="small">Portfolio</Button>
                                </Link>
                            </li>
                        </ul>
                    </nav>
                </Box>
                <Drawer anchor="left" open={isNavbar} onClose={() => { setNavbar(!isNavbar) } }>
